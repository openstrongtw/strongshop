...
namespace App\Http\Controllers\Strongadmin\Article;
...
class CategoryController extends BaseController
{
     ...
     * @param  \App\Models\Admin\Product\Category  $category
    ...
    public function index(Request $request, Category $category)
    {
        ...
        $model->with('parent');
        $rows = $model->get();
        return $this->view('article.category.index', ['rows' => $rows]);
    }
    ...
    public function update(Request $request, Category $category)
    {
        if (!$request->expectsJson())
        {
            ...
            return $this->view('article.category.form', ['model' => $model, 'menus' => $menus]);
        }
        $rules = array_merge_recursive($category->rules(), [
            'id' => ['required', 'integer', Rule::exists('category')],
        ]);
    ...
    {
        $validator = Validator::make($request->all(), [
                    'id' => ['required', new NotExists('product_category', 'category_id')],
        ]);
        if ($validator->fails())
        {
            return ['code' => 3001, 'message' => $validator->errors()->first(), 'data' => $validator->errors()];
