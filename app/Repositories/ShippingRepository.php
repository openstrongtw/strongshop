...
class ShippingRepository
{
    ...
    public static function getFee($weight, array $config)
    {
        ...
        $range_max_weight = end($range_configs)['range']['end']; //重量範圍內最大重量值
        $over_weight = $weight <= $range_max_weight ? 0 : ($weight - $range_max_weight); //超出`重量範圍`后的重量
        //如果超出`重量範圍`
        if ($weight > $range_max_weight)
        {
            $over_weight = $weight - $range_max_weight; //超出`重量範圍`后的重量
            $weight = $range_max_weight; //重置重量
            $fee += ceil($over_weight / $config['over_weight']['weight']) * $config['over_weight']['amount']; //超出`重量範圍`后的費用
        }
